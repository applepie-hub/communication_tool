  ...
  switch (user.status['@type']) {
    ...
    case 'userStatusOffline': {
      ...
      if (diff.getTime() / 1000 < 60 * 60) {
        ...
        return ` ${minutes === 1 ? '1分钟' : minutes + '分钟'}前在线`;
      }
      ...
      if (wasOnline > today) {
        ...
        if (diff.getTime() / 1000 < 6 * 60 * 60) {
          ...
          return `${hours === 1 ? '1小时' : hours + '小时'}前在线`;
        }
        ...
        return `最后一次在线是 ${dateFormat(wasOnline, 'H:mm')}`;
      }
      ...
      if (wasOnline > yesterday) {
        ...
        return `昨天在线 ${dateFormat(wasOnline, 'H:mm')}`;
      }
      ...
      return `最近一次在线日期: ${dateFormat(wasOnline, 'yyyy.MM.dd')}`;
    }
